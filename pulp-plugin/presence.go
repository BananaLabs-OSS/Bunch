...
// their friends. Runs inside the plugin, owns no goroutines (all
...
type Hub struct {
	...
	// connToAccount is the reverse lookup for dispatch on close.
	connToAccount map[uuid.UUID]uint64
...
func (h *Hub) notifyFriends(accountID uuid.UUID, msgType string) {
	...
	if err != nil {
		fmt.Printf("[bunch] presence list friends error: %v\n", err)
		return
	}
	...
	h.mu.Lock()
	targets := make([]uint64, 0, len(friendIDs))
	...
	for _, connID := range targets {
		if err := pulp.WS.Send(pulp.WSSendRequest{
			...
			fmt.Printf("[bunch] ws_send to conn %d failed: %v\n", connID, err)
		}
	}
}
